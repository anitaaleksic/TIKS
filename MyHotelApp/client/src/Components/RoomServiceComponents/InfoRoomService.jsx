...
export default function InfoRoomService() {
  ...
  useEffect(() => {
    axios.get(`/api/RoomService/GetRoomServiceByName/${itemName}`)
      ...
      .catch(err => {
        console.error(err);
        setError('Failed to load room service details.');
      });
  }, [itemName]);
  ...
  return (
    <div className="extraservice-form">
      <h2>Room Service Info</h2>

      <div className="form-group">
        <label className="form-label">Name:</label>
        <p>{service.itemName}</p>
      </div>

      <div className="form-group">
        <label className="form-label">Price:</label>
        <p>${service.itemPrice.toFixed(2)}</p>
      </div>
